...
count = 0
for r in reports:
    print("Working hard! Current repoort =",count,"of",len(reports)," -- ",(count/len(reports))*100,"%",end='\r')
    ...
    nextValue = 0
    while difference > 0:
        nextValue = differences[difference-1][0] - differences[difference][0]
        #print("nV=",nextValue)
        newDiffs = differences[difference-1]
        newDiffs.insert(0,nextValue)
